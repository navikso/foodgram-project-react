...
from api.serializers.subscription.show import SubscriptionShowSerializer
...
class SubscriptionListView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return Response({
            ...
            "results": SubscriptionListSerializer(outcome.result["results"],
                                          many=True).data
...
class SubscriptionCreateDeleteView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        outcome = ServiceOutcome(SubscriptionCreateService, kwargs | {"user": request.user})
        return Response(SubscriptionUserGetSerializer(outcome.result).data, status=status.HTTP_201_CREATED)
