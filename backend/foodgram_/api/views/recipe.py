...
class RecipeListCreateView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return Response({
            ...
            "results": RecipeListSerializer(outcome.result["results"], many=True).data
        }, status=status.HTTP_200_OK)

    def post(self, request, *args, **kwargs):
        data = request.data if not isinstance(request.data, QueryDict) else request.data.dict()
        outcome = ServiceOutcome(RecipeCreateService, data | {"user": request.user}, data)
        return Response(RecipeShowSerializer(outcome.result).data, status=status.HTTP_201_CREATED)
...
class RecipeDownloadView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        try:
            outcome = ServiceOutcome(RecipeDownloadService, {"user": request.user})
...
class RecipeGetDeleteUpdateView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return Response(RecipeListSerializer(outcome.result).data, status=status.HTTP_200_OK)
    ...
    def patch(self, request, *args, **kwargs):
        ...
        return Response(RecipeShowSerializer(outcome.result).data, status=status.HTTP_200_OK)
